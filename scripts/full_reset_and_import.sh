...
# Use Muditor's bun-based Prisma CLI (supports Prisma 7 prisma.config.ts)
MUDITOR_DB_DIR="$(cd "$(dirname "$0")/../../muditor/packages/db" && pwd)"
# The consent env var is required when Prisma detects AI agent invocation
PRISMA_CONSENT="Run the fierylib importer with players"
if ! (cd "$MUDITOR_DB_DIR" && PRISMA_USER_CONSENT_FOR_DANGEROUS_AI_ACTION="$PRISMA_CONSENT" bunx prisma migrate reset -f); then
  echo "⚠️  Migrate reset failed, trying db push with force-reset..."
fi

echo ""
echo "[2.2] Ensuring schema is applied..."
if ! (cd "$MUDITOR_DB_DIR" && bunx prisma db push --accept-data-loss); then
  echo "⚠️  db push failed, trying with force-reset..."
  (cd "$MUDITOR_DB_DIR" && PRISMA_USER_CONSENT_FOR_DANGEROUS_AI_ACTION="$PRISMA_CONSENT" bunx prisma db push --force-reset --accept-data-loss)
fi

echo ""
echo "[2.3] Generating Prisma Python client..."
# prisma-client-py (v5) requires url in datasource block, but Prisma 7 moved it to
# prisma.config.ts. Create a temporary schema with the url injected for generation.
SCHEMA_PATH="${PRISMA_SCHEMA_PATH:-../muditor/packages/db/prisma/schema.prisma}"
TEMP_SCHEMA=$(mktemp /tmp/schema_fierylib_XXXXXX.prisma)
sed 's/datasource db {/datasource db {\n  url = env("DATABASE_URL")/' "$SCHEMA_PATH" > "$TEMP_SCHEMA"
poetry run prisma generate --generator py --schema="$TEMP_SCHEMA"
rm -f "$TEMP_SCHEMA"
...
# Step 5: Seed game settings (config, levels, text/MOTD)
if [[ "$SKIP_IMPORT" -eq 0 ]] && [[ -z "$DRY_RUN" ]]; then
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "Step 5: Seeding game settings (config, levels, text)"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo ""

  SETTINGS_CMD="poetry run fierylib seed game-settings"

  if [[ -n "$VERBOSE" ]]; then
    SETTINGS_CMD="$SETTINGS_CMD --verbose"
  fi
  ...
  if [[ -n "$DRY_RUN" ]]; then
    echo "⏭️  Skipping game settings seeding (dry-run mode)"
  else
    echo "⏭️  Skipping game settings seeding (world import skipped)"
  fi
  echo ""
fi

# Step 6: Seed help entries from legacy help files
if [[ "$SKIP_IMPORT" -eq 0 ]] && [[ -z "$DRY_RUN" ]]; then
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "Step 6: Importing help entries from legacy help files"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo ""

  HELP_CMD="poetry run fierylib seed help-entries --clear"

  if [[ -n "$VERBOSE" ]]; then
    HELP_CMD="$HELP_CMD --verbose"
  fi

  eval $HELP_CMD

  echo ""
  echo "✅ Help entries imported"
  echo ""
else
  if [[ -n "$DRY_RUN" ]]; then
    echo "⏭️  Skipping help entries import (dry-run mode)"
  else
    echo "⏭️  Skipping help entries import (world import skipped)"
  fi
  echo ""
fi

# Step 7: Seed MUD command definitions
if [[ "$SKIP_IMPORT" -eq 0 ]] && [[ -z "$DRY_RUN" ]]; then
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "Step 7: Seeding MUD command definitions"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo ""

  COMMANDS_CMD="poetry run fierylib seed commands"

  if [[ -n "$VERBOSE" ]]; then
    COMMANDS_CMD="$COMMANDS_CMD --verbose"
  fi

  eval $COMMANDS_CMD

  echo ""
  echo "✅ Command definitions seeded"
  echo ""
else
  if [[ -n "$DRY_RUN" ]]; then
    echo "⏭️  Skipping command seeding (dry-run mode)"
  else
    echo "⏭️  Skipping command seeding (world import skipped)"
  fi
  echo ""
fi
...
if [[ "$SKIP_PLAYERS" -eq 0 ]] && [[ -z "$DRY_RUN" ]] && { [[ -n "$WITH_PLAYERS" ]] || [[ -n "$PLAYERS_LIST" ]]; }; then
  ...
  poetry run fierylib import-quests

  echo ""
  echo "✅ Quest import complete"
  echo ""
else
