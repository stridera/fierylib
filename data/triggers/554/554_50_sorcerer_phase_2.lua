-- Trigger: sorcerer_phase_2
-- Zone: 554, ID: 50
-- Type: MOB, Flags: RECEIVE
-- Status: NEEDS_REVIEW
--   Syntax error: luac: <sorcerer_phase_2>:111: function arguments expected near ']'
--   Complex nesting: 15 if statements
--   Large script: 10465 chars
--
-- Original DG Script: #55450

-- Converted from DG Script #55450: sorcerer_phase_2
...
-- 
if (actor.class == "sorcerer" or actor.class == "cryomancer" or actor.class == "pyromancer") &actor.level >= 21 &actor:get_quest_stage("phase_armor") >= 2 then
    -- 
    -- pertinient object definitions for this class
    -- destroyed armor
    -- gems for this class
    -- rewards for this class
    local id_reward_helm = 75
    local id_reward_arms = 76
    local id_reward_chest = 77
    local id_reward_legs = 78
    local id_reward_boots = 79
    local id_reward_bracer = 80
    local id_reward_gloves = 81
    -- 
    ...
    if not is_armor then
        ...
        -- 
        if gems == 3 &actor.quest_variable[phase_armor:team_a_idrmor_armor_acquired] == 1 then
            wait(2)
        ...
        -- check to see if the quest is complete and the reward can be given..
        -- 
        if armor == 1 &actor.quest_variable[phase_armor:id_gem_gems_acquired] == 3 then
            wait(2)
            actor:send(tostring(self.name) .. " tells you, \"Excellent intrepid adventurer, you have provided me with all\"")
            actor:send(tostring(self.name) .. " tells you, \"I need in order to reward you with " .. "%get.obj_shortdesc[%id_reward%]%!\"")
            wait(1)
