...
if actor.alignment <= 150 then
    if actor:get_quest_stage("Black_Legion") > 0 then
        ...
        if actor:get_quest_var("black_legion:bl_faction") >= 85 then
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_gloves%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_gloves%]%</>")
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_symbol%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_symbol%]%</>")
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_light%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_light%]%</>")
        end
        if actor:get_quest_var("black_legion:bl_faction") >= 100 then
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_belt%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_belt%]%</>")
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_shiv%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_shiv%]%</>")
        end
        if actor:get_quest_var("black_legion:bl_faction") >= 115 then
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_boots%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_boots%]%</>")
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_lsword%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_lsword%]%</>")
        end
        if actor:get_quest_var("black_legion:bl_faction") >= 130 then
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_legs%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_legs%]%</>")
        end
        if actor:get_quest_var("black_legion:bl_faction") >= 145 then
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_robe%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_robe%]%</>")
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_jerkin%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_jerkin%]%</>")
        end
        if actor:get_quest_var("black_legion:bl_faction") >= 160 then
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_mask%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_mask%]%</>")
            actor:send("- <b:yellow>" .. "%get.obj_shortdesc[%vnum_3bl_smace%]%</> for <magenta>%get.obj_shortdesc[%vnum_gem_3bl_smace%]%</>")
        end
        if actor:get_quest_var("black_legion:bl_faction") < 160 then
            -- (empty send to actor)
