...
-- Status: NEEDS_REVIEW
--   Syntax error: luac: <waterform_waters_examine>:4: function arguments expected near '.'
--
...
if actor:get_quest_stage("waterform") == 6 and (actor:has_equipped("2808") or actor:has_item("2808")) then
    if actor.quest_variable[waterform:self.vnum] then
        wait(1)
        ...
        actor.name:set_quest_var("waterform", "%self.vnum%", 1)
