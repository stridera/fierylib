...
-- Status: CLEAN
--   Note: Displays reward list for Black Legion faction
--
...
if actor.alignment <= 150 then
    if actor:get_quest_stage("Black_Legion") > 0 then
        ...
        if actor:get_quest_var("black_legion:bl_faction") >= 20 then
            ...
            actor:send("- <b:yellow>" .. obj_name(vnum_3bl_food) .. "</> for <magenta>" .. obj_name(vnum_gem_3bl_food) .. "</>")
            actor:send("- <b:yellow>" .. obj_name(vnum_3bl_drink) .. "</> for <magenta>" .. obj_name(vnum_gem_3bl_drink) .. "</>")
            actor:send("- <b:yellow>" .. obj_name(vnum_3bl_cap) .. "</> for <magenta>" .. obj_name(vnum_gem_3bl_cap) .. "</>")
            actor:send("- <b:yellow>" .. obj_name(vnum_3bl_ssword) .. "</> for <magenta>" .. obj_name(vnum_gem_3bl_ssword) .. "</>")
        end
        if actor:get_quest_var("black_legion:bl_faction") >= 40 then
            actor:send("- <b:yellow>" .. obj_name(vnum_3bl_neck) .. "</> for <magenta>" .. obj_name(vnum_gem_3bl_neck) .. "</>")
            actor:send("- <b:yellow>" .. obj_name(vnum_3bl_staff) .. "</> for <magenta>" .. obj_name(vnum_gem_3bl_staff) .. "</>")
        end
        if actor:get_quest_var("black_legion:bl_faction") >= 55 then
            actor:send("- <b:yellow>" .. obj_name(vnum_3bl_arm) .. "</> for <magenta>" .. obj_name(vnum_gem_3bl_arm) .. "</>")
            actor:send("- <b:yellow>" .. obj_name(vnum_3bl_whammer) .. "</> for <magenta>" .. obj_name(vnum_gem_3bl_whammer) .. "</>")
        end
        if actor:get_quest_var("black_legion:bl_faction") >= 70 then
            actor:send("- <b:yellow>" .. obj_name(vnum_3bl_wrist) .. "</> for <magenta>" .. obj_name(vnum_gem_3bl_wrist) .. "</>")
            actor:send("- <b:yellow>" .. obj_name(vnum_3bl_flail) .. "</> for <magenta>" .. obj_name(vnum_gem_3bl_flail) .. "</>")
        end
        if actor:get_quest_var("black_legion:bl_faction") < 70 then
