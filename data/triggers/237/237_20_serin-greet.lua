...
if actor.is_player then
    if actor:get_quest_stage("sunfire_rescue") == 0 then
        actor:send("The prisoner looks up at " .. tostring(actor.name) .. " hopefully.")
        actor:send("The prisoner says, 'Can you help me?'")
    elseif actor:get_quest_stage("sunfire_rescue") == 1 then
        ...
        actor:send("The prisoner says, 'Ah! Have you brought me the treasures from Templace?'")
        actor:send("'Please, give them to me! If you forgot what you need to do, you can ask me")
        actor:send("'for a <b:white>[progress]</> report.'")
    end
end