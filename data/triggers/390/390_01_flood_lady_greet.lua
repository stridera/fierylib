...
--
-- The Lady of the Sea greets arrivals. High-level Cryomancers who haven't
-- started the quest are singled out as her would-be Envoy; players already
-- on the quest are nagged about progress.

wait(1)
local stage = actor:get_quest_stage("flood")
if stage == 0 then
    if string.find(actor.class, "Cryomancer") and actor.level > 80 then
        self:command("point " .. actor.name)
