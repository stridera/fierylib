...
if actor:get_quest_stage("illusionist_subclass") == 4 then
    ...
    doors.set_state(get_room(363, 39), "east", {action = "room"})
    doors.set_description(get_room(363, 39), "east", "A narrow walkway zigzags through the air.")
    wait(8)
    self.room:send("The walkway fades, revealing nothing but a misty drop into the waters below.")
    doors.set_state(get_room(363, 39), "east", {action = "room"})
    doors.set_description(get_room(363, 39), "east", "The balcony ends at the waterfall.  Oddly, there is no railing at this end.")
end