-- Trigger: phase_2_monk_helm
-- Zone: 553, ID: 84
-- Type: MOB, Flags: SPEECH_TO
-- Status: CLEAN
--
-- Original DG Script: #55384

-- Converted from DG Script #55384: phase_2_monk_helm
...
if actor.class == "monk" and actor.level >= 21 and actor:get_quest_stage("phase_armor") >= 2 then
    wait(2)
    actor:send(tostring(self.name) .. " tells you, \"A cap for ya? Well now I can make a nice cap for the monk types\"")
    actor:send(tostring(self.name) .. " tells you, \"but I'll need 3 enchanted ambers, and a Burned Cap.\"")
