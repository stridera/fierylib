...
-- the banish quest if they're at stage 6 (seraph) and reveal the next letter.
local size = actor.group_size or 0
if size > 0 then
    for i = 1, size do
        local person = actor.group_member[i]
        if person and person.room == self.room
                and person:get_quest_stage("banish") == 6 then
            person:advance_quest("banish")
            person:send("<b:magenta>A single letter pops into your mind - <b:cyan>P</>")
        end
    end
elseif actor:get_quest_stage("banish") == 6 then
    actor:advance_quest("banish")
    actor:send("<b:magenta>A single letter pops into your mind - <b:cyan>P</>")
end