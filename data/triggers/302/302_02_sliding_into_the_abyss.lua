...
-- Converted from DG Script #30202: Sliding into the abyss
...
if victim.class == "Ranger" or victim.class == "Druid" then
    self.room:send_except(victim, "A rock shifts under " .. tostring(victim.name) .. "'s foot, but " .. tostring(victim.name) .. " quickly")
    -- Fragment (possible truncation): steps to one side
...
victim:teleport(get_room(302, 55))
-- victim looks around