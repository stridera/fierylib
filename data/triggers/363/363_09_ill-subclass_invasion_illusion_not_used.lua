...
-- zone.echo broadcasts to every room in the zone (including this one),
-- so the redundant self.room:send calls from the original DG conversion
-- have been removed.
wait(3)
...
-- 36301 -> (zone_id 363, local_id 1) Gannigan
local gannigan = nil
local quester = nil
...
if quester and gannigan then
    if quester:get_quest_stage("illusionist_subclass") == 2 then
        -- TODO: original DG advanced the quest twice in a row; preserved
        -- as-is since this trigger is flagged NOT USED. Verify intent
        -- (skip stage 3 entirely?) before reactivating.
    ...
    elseif quester:get_quest_stage("illusionist_subclass") == 3 then
        -- TODO: same double-advance pattern as above; verify before reuse.
